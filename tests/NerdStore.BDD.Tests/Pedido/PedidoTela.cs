...
public class PedidoTela : PageObjectModel
{
    ...
    {
        Helper.IrParaUrl(Helper.Configuration.VitriniUrl);
    }
    ...
    public int ObterQuantidadeNoEstoque()
    {
        ...
        if (char.IsNumber(quantidade.ToString(), 0))
        {
            return quantidade;
        }
    ...
    public decimal ObterValorUnitarioProdutoCarrinho()
    {
        return Convert.ToDecimal(Helper
            .ObterTextoElementoPorId("valorUnitario")
            .Replace("$", string.Empty)
            .Replace(",", string.Empty)
            .Trim());
    }

    public decimal ObterValorTotalDoCarrinho()
    {
        return Convert.ToDecimal(Helper
            .ObterTextoElementoPorId("valorTotalCarrinho")
            .Replace("$", string.Empty)
            .Replace(",", string.Empty)
            .Trim());
    }
    ...
    {
        return Helper.ObterTextoElementoPorId("alert-danger");
    }
    ...
    public string ObterIdPrimeiroProdutoCarrinho()
    {
        return Helper.ObterElementoPorXPath("/html/body/div/main/div/div/div/table/tbody/tr[1]/td[1]/div/div/h4/a")
            .GetAttribute("href");//TODO verify if is href
    }

    public void GarantirQueOPrimeiroItemDaVitriniEstejaAdicionado()
    {
        NavegarParaCarrinhoDeCompras();
        if (ObterValorTotalDoCarrinho() > 0) return;
    ...
    {
        return Convert
            .ToInt32(Helper.ObterValorTextBoxPorId("qauntidade"));
    }
    ...
    {
        while (ObterValorTotalDoCarrinho() > 0)
        {
